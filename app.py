...
class FlashAlphaClient:
    ...
    # --- GEX across all expirations, for the 3D surface --------------------
    def fetch_gex_surface(self, symbol: str) -> pd.DataFrame:
        """Same shape as fetch_gex but spanning every available expiration."""
        return self.fetch_gex(symbol, dte=None)
...
@st.cache_data(ttl=60, show_spinner=False)
def cached_gex_surface(symbol: str) -> pd.DataFrame:
    return get_client().fetch_gex_surface(symbol)
...
def build_gex_surface(df: pd.DataFrame, symbol: str) -> go.Figure:
    pivot = df.pivot_table(index="dte", columns="strike", values="net_gex", aggfunc="sum").fillna(0)
    fig = go.Figure(
        data=[go.Surface(z=pivot.values, x=pivot.columns, y=pivot.index, colorscale="RdYlGn")]
    )
    fig.update_layout(
        title=f"{symbol} 3D Gamma Exposure Surface",
        scene=dict(
            xaxis_title="Strike",
            yaxis_title="DTE",
            zaxis_title="Net GEX",
...
with st.sidebar:
    ...
    model = st.radio(
        "Select a model",
        ["GEX Bar Chart", "3D GEX Surface", "NQ Futures Candlestick"],
    )
...
# --- Model 2: 3D GEX Surface -------------------------------------------------
elif model == "3D GEX Surface":
    st.subheader("3D Surface Gamma Exposure Model")
    st.caption("Pulls GEX across all available expirations and plots Strike × DTE × Net GEX.")

    if st.button("Fetch & Render Surface", type="primary"):
        with st.spinner("Fetching multi-expiration GEX data from FlashAlpha..."):
            try:
                df = cached_gex_surface(symbol)
            except FlashAlphaAPIError as e:
                st.error(f"FlashAlpha API error: {e}")
            else:
                st.plotly_chart(build_gex_surface(df, symbol), use_container_width=True)
                with st.expander("Raw data"):
                    st.dataframe(df, use_container_width=True)

# --- Model 3: NQ Futures Candlestick -----------------------------------------
else:
